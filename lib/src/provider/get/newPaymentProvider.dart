...
class NewPaymentProvider with ChangeNotifier {
  int price = 0;
  List<Pays> _pays = [];
  bool error = false;
  ...
  Future<GetPaidPaymentModel> getNewPayment(String token) async {
    final List<Pays> loadedOrders = [];
    Map<String, String> headers = {
      "X-localization": localization.currentLanguage.toString(),
      "Authorization": "Bearer $token"
    };
    Response response = await _utils.get("get-new-payment", headers: headers);
    if (response.statusCode == 200) {
      print("get get-new-payment sucsseful");
      ...
      print("error get get-new-payment data");
      error = true;
      notifyListeners();
      return GetPaidPaymentModel.fromJson(response.data);
    }
